...
def setup_handlers(web_app, notebook_dir):
    base_url = web_app.settings['base_url']
    manager = None
    
    try:
        from .tensorboard_manager import TensorboardManager
        manager = TensorboardManager(notebook_dir)
        
    except ImportError as e:
        logger.info("import tensorboard error, check tensorflow install", exc_info=e)
        handlers = [
            (ujoin(
        ...
        handlers = [
            ...
            (ujoin(
                ...
                api_handlers.TbRootHandler, dict(notebook_dir=notebook_dir)),
            (ujoin(
                base_url, r"/api/tensorboard/(?P<name>\w+)"),
                api_handlers.TbInstanceHandler, dict(notebook_dir=notebook_dir)),
            (ujoin(
